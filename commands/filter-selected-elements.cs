...
public abstract class FilterElementsBase
{
    ...
    public void Execute()
    {
        ...
        try
        {
            ...
            // Create a unique key for each entity to map back to full data
            var entityDataMap = new Dictionary<string, Dictionary<string, object>>();
            ...
            for (int i = 0; i < entityData.Count; i++)
            {
                ...
                // Create a unique key
                string uniqueKey = $"{data["Handle"]}_{data["DocumentName"]}_{i}";
                
                // Store the full data in our map
                entityDataMap[uniqueKey] = data;
            ...
            var propertyNames = displayData.First().Keys
                .Where(k => !k.EndsWith("ObjectId") && k != "UniqueKey")
                .ToList();
            ...
            foreach (var row in chosenRows)
            {
                ...
                if (!entityDataMap.TryGetValue(uniqueKey, out var fullData))
                    continue;
                
                // Check if this is an external entity
                if (fullData.TryGetValue("IsExternal", out var isExternal) && (bool)isExternal)
                {
                    externalEntities.Add(fullData);
                }
                else if (fullData.TryGetValue("ObjectId", out var objIdObj) && objIdObj is ObjectId objectId)
                {
...
{
    [CommandMethod("filter-selected-elements")]
    public void FilterSelectedElementsCommand()
    {
        var command = new FilterSelectedElementsImpl();
        command.Execute();
    }

    [CommandMethod("filter-selected-elements-span")]
    public void FilterSelectedElementsSpanCommand()
    {
        var command = new FilterSelectedElementsSpanImpl();
        command.Execute();
    }
}

public class FilterSelectedElementsImpl : FilterElementsBase
{
...
public class FilterSelectedElementsSpanImpl : FilterElementsBase
{
