...
public class SelectBySameCategoriesOfSelected
{
    public static void ExecuteViewScope(Editor ed)
    {
        ...
        try
        {
            ...
            if (selResult.Status == PromptStatus.Error)
            {
                ...
                selectionOpts.MessageForAdding = "\nSelect entities to get their categories: ";
            ...
            // Collect categories from currently selected entities
            var categoriesToSelect = new HashSet<string>(StringComparer.OrdinalIgnoreCase);

            // Get categories from selected entities
            ...
            if (categoriesToSelect.Count == 0)
            {
                ed.WriteMessage("\nNo valid categories found in selected entities.\n");
                return;
            }

            ed.WriteMessage($"\nFound {categoriesToSelect.Count} unique category(ies): {string.Join(", ", categoriesToSelect)}\n");
            ...
            using (var tr = db.TransactionManager.StartTransaction())
            {
                ...
                foreach (ObjectId id in currentSpace)
                {
                    try
                    {
                        ...
                        {
                            var category = SelectByCategories.GetEntityCategory(entity);
                            if (categoriesToSelect.Contains(category))
                            {
            ...
            if (currentDocumentIds.Count > 0)
            {
                ...
                ed.WriteMessage($"\nSelected {currentDocumentIds.Count} entities in category(ies): {string.Join(", ", categoriesToSelect)}\n");
            }
            else
            {
                ed.WriteMessage($"\nNo entities found in category(ies): {string.Join(", ", categoriesToSelect)}\n");
            }
        }
    ...
    public static void ExecuteDocumentScope(Editor ed, Database db)
    {
        ...
        try
        {
            ...
            // Collect categories from stored selection
            var categoriesToSelect = new HashSet<string>(StringComparer.OrdinalIgnoreCase);

            // Get categories from stored selection items in current document
            ...
            if (categoriesToSelect.Count == 0)
            {
                ed.WriteMessage("\nNo valid categories found in stored selection.\n");
                return;
            }
            ...
            {
                ed.SetImpliedSelection(currentDocumentIds.ToArray());
                ed.WriteMessage($"\nSelected {currentDocumentIds.Count} entities in category(ies): {string.Join(", ", categoriesToSelect)} from current document\n");
            }
    ...
    public static void ExecuteApplicationScope(Editor ed)
    {
        ...
        try
        {
            ...
            // Get categories from stored selection items
            foreach (var item in storedSelection)
            {
                try
                {
                    ...
                    {
                        // Different document - try to get category from external document
                        var category = GetCategoryFromExternalDocument(item.DocumentPath, item.Handle);
                        if (!string.IsNullOrEmpty(category))
                        {
                            categoriesToSelect.Add(category);
                        }
            ...
            SelectEntitiesFromAllDocuments(categoriesToSelect, ed);
        }
        catch (System.Exception ex)
        {
            ed.WriteMessage($"\nError: {ex.Message}\n");
    ...
    private static string GetCategoryFromExternalDocument(string documentPath, string handle)
    {
        try
        {
            ...
            // Get the category from the entity
            if (externalDoc != null)
            {
                try
                {
                    ...
                    if (objectId != ObjectId.Null)
                    {
                        using (var tr = externalDoc.Database.TransactionManager.StartTransaction())
                        {
                            ...
                            {
                                tr.Commit();
                                return SelectByCategories.GetEntityCategory(entity);
                            }
                            tr.Commit();
                        }
                    }
    ...
    private static void SelectEntitiesFromAllDocuments(HashSet<string> categoriesToSelect, Editor ed)
    {
        var docs = AcadApp.DocumentManager;
        ...
        if (totalCount > 0)
        {
            ...
            ed.WriteMessage($"\nSelected {totalCount} entities in category(ies): {string.Join(", ", categoriesToSelect)} from {documentCount} document(s)\n");
